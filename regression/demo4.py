# -*- coding: utf-8 -*-
"""
Created on Wed Dec 20 21:15:21 2017
...
def search(x_l,x_h,y_l,y_h,num):
    ...
    for i in range(num):
        print('ff')
        if neigh.predict([x_r[i],y_r[i]]) > temp:
            temp = neigh.predict([x_r[i],y_r[i]])
            x_temp = x_r[i]
            y_temp = y_r[i]
        else:
            print(i)
...
x_r = np.arange(2,4,(4-2)/float(2))


d4 = math.sqrt((w-c1[0])**2 + (e-c1[1])**2)
